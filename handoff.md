...
- **版式规范（一张卡全卡一致）**：① 标签列一律 `66px`（≤400px 收 `58px`）——`.snap-line` / `.snap-part` / `.snap-line.acts` 的金额左边界因此都落在同一竖线上；② 小节标题 = `.snap-sec`（自带 1px 实线上边框），不再并存「空虚线行 + 三列小字标题」两套写法；③ 从属关系一律「缩进 + 左 2px 竖线」（`.snap-parts` / `.snap-tbl`），层级不靠堆虚线表达；④ 明细区三级：列头（400 淡）/ 分组头（700 + 实线 + 更亮）/ 合计（700 + 实线）。
- **两种网格是刻意分工**，不是漏统一：金额行 `66px 1fr`（左对齐，读「变了多少」）；明细 / 分组 / 合计 `1fr 78px 84px`（右对齐，逐只对账）。
- **线的语义**：实线 = 区块分界；虚线 = 卡片边界（卡内不出现虚线）。
- **折叠默认值**：期次解读（备注）默认折叠成一行——首段截断 + `⚠ N · M 字`，收起不丢「有没有口径警示」与体量；场内/场外分组、更早期数、清仓全部明细、操作日志同理。全部用原生 `<details>`，零 JS 状态。
- **打印约束**：`@media print` 用 `details > *:not(summary) { display: revert }` 强制展开全部折叠件；**`summary` 不能整行 `display:none`** ——「场内/场外」小计、`⚠ N · M 字`、对账差异条数都长在 summary 上，藏掉会让打印稿少了对账锚点。
