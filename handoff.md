...
### 3.4 `asOf` 时区换算
页头下方时区换算条由 `us / et / cn / tz / local` 五个字符串组成（美股交易日 / 美东数据时刻 / 对应北京时间 / 时区标注 / 本地更新时间）。`et` 为脚本抓取时的美东时刻（盘中=抓取时间，收盘=16:00），页面按 `intraday` 标志自动显示「盘中快照 / 收盘数据」，无需手改。更新时**照抄 data.js 上一次的格式改数字即可**。唯一需记忆的规则：每年 **3 月第二个周日 → 11 月第一个周日**为夏令时（美东落后北京 12h），其余时间冬令时（落后 13h）。
