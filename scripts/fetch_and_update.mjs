...
if (okCore) {
  ...
  /* 纳指/标普 ETF 场内价口径（持仓页水位梯/买入分布用，与交割流水同口径） */
  const ETF_KEYS2 = { "159941": "etfNdx", "513650": "etfSpx" };
  for (const [code, key] of Object.entries(ETF_KEYS2)) {
    const v = etfRangeV[code];
    if (!v) continue;
    const line = `  ${key}: { close: ${v.close.toFixed(3)}, chg: ${v.chg.toFixed(2)}, low52: ${v.low52.toFixed(3)}, ath: ${v.ath.toFixed(3)}, athDate: "${v.athDate}" },  // ${key} 持仓（场内价口径，AUTO）`;
    apply(new RegExp(`^  ${key}:\\s*\\{[^\\n]*$`, "m"), line, key);
  }
  if (premiumsV) {
